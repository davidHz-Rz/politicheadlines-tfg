...
def build_vlm_text_ranker(base_ensemble: str, vlm_text_base: str):
    """Construye la base textual que se fusionará con los scores visuales.

    - ensemble: usa directamente el ensemble textual especificado.
    - tail_rank10: aplica TailReranker sobre el ensemble usando bert_rank10 para la cola.

    Nota: TailReranker devuelve puntuaciones ordinales coherentes con su ranking final,
    por lo que pueden reutilizarse como text_scores_list para la fusión VLM.
    """
    ...
    if vlm_text_base == "tail_rank10":
        ...
        notes = f"vlm_text_base=tail_rank10; base={base_ensemble}; tail=bert_rank10; top_k=10"
        return tail_rank10, name, notes

    raise ValueError(
        f"vlm_text_base no soportado: {vlm_text_base!r}. "
        "Usa 'ensemble' o 'tail_rank10'."
    )
...
def stage_vlm(
    ...
    for backend, model_name in VLM_MODELS:
        ...
        # Fusión con la base textual seleccionada.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="ensemble",
        help=(
            "Base textual para fusionar con el VLM. "
            "'ensemble' usa el ensemble indicado en --base-ensemble; "
            "'tail_rank10' usa ese ensemble seguido de TailReranker con bert_rank10."
        ),
    )
